...
for(amount = 0; amount < NUM; amount++)   {
        for(count = 0; count < NUM-1; count++)      {                  
             for(inner = 0; inner < LEN-1; inner++) {        
                if(Strings[count][inner] > Strings[count+1][inner])     {
                        for(in = 0; in < LEN; in++)        {
                        char temp = Strings[count][in];
                        Strings[count][in] = Strings[count+1][in];
                        Strings[count+1][in] = temp;
                }  
                }
                if(Strings[count][inner] < Strings[count+1][inner])     {
                break;
